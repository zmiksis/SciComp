...
int main(){
  ...
  for(int i = 1; i <= N; ++i){
    ...
    for(int i = 0; i < exponents.size(); ++i){
      if(exponents[i] > 0){
        phi *= pow(prime_numbers[i],exponents[i]) - pow(prime_numbers[i],exponents[i] - 1);
      }
    }
